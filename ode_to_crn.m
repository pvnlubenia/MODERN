...
%           - reaction: a list of all reactions/fluxes in the system, each    %
...
function [ode] = ode_to_crn(ode)
    ...
    %
    % STEP 1: Add to ode.species all species indicated in the reactions, i.e., the state variables
    %
    ...
    %
    % STEP 2: Add to ode.flux all fluxes indicated in the reactions
    %
    ...
    for i = 1:numel(ode.reaction)
        ...
        for j = 1:numel(ode.reaction(i).species)
            A(end, find(strcmp(ode.reaction(i).species{j}, ode.species))) = ode.reaction(i).order{j};
        end
    end
